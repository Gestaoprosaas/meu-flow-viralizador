...
export default function MarketingFooter({ onStart }: MarketingFooterProps) {
  ...
  return (
    <footer id="marketing-footer-root" className="relative w-full bg-[#111118] border-t border-[#1E1E2E] overflow-hidden font-sans">
      ...
      <div className="max-w-7xl mx-auto py-16 px-6 md:px-12 grid grid-cols-1 md:grid-cols-12 gap-12 relative z-10 text-left font-sans">
        <div className="md:col-span-5 space-y-4">
          <div className="flex items-center gap-2">
            ...
            <span className="font-extrabold text-white text-sm tracking-widest uppercase">
              ViralForge <span className="text-[#69C9D0]">AI</span>
            </span>
          </div>
      ...
      <div className="max-w-7xl mx-auto py-8 px-6 md:px-12 border-t border-white/[0.04] flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-gray-600 relative z-10 font-mono">
        <div>
          &copy; {currentYear} ViralForge AI. Todos os direitos reservados.
        </div>
        <div className="flex gap-4">
          <span>Brasília, DF, Brasil</span>
          <span>&middot;</span>
          <button onClick={handleScrollToTop} className="hover:text-white transition cursor-pointer font-bold">Voltar ao Topo &uarr;</button>
        </div>
      </div>
    </footer>
  );
